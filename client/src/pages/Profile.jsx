...
const Profile = () => {
  ...
  const sentTxs = transactions.filter(t => t.senderUPI === upiId).slice(-8);
  const receivedTxs = transactions.filter(t => t.receiverUPI === upiId).slice(-8);
  ...
  const allTxs = [...transactions].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)).slice(-8);

  const chartData = {
    labels: allTxs.map(t => moment(t.timestamp).format('DD MMM')),
    datasets: [
      {
        label: 'Amount',
        data: allTxs.map(t => Number(t.amount)),
        backgroundColor: allTxs.map(t => t.senderUPI === upiId ? 'rgba(251, 146, 60, 0.85)' : 'rgba(139, 92, 246, 0.85)'),
        borderRadius: 6,
        barThickness: 14,
      }
    ]
          ...
          <div className="relative bg-[#0c0c0c]/80 backdrop-blur-xl border border-zinc-800/60 rounded-[2rem] p-8 shadow-2xl overflow-hidden">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center py-2 border-b border-zinc-800/40">
                <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Mobile</span>
                <span className="text-zinc-300 text-sm font-semibold">{mob}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-zinc-800/40">
                <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Date of Birth</span>
                <span className="text-zinc-300 text-sm font-semibold">{dob}</span>
              </div>
            </div>
              ...
              <div className="absolute inset-0 bg-[#0a0a0a]/95 backdrop-blur-2xl z-50 flex flex-col p-8 rounded-[2rem]">
                ...
                <div className="space-y-4 flex-1">
                  ...
                  <div>
                    ...
                    <input type="text" value={mob} onChange={e => setMob(e.target.value)} className="w-full py-3 px-4 bg-zinc-900 border border-zinc-800 rounded-xl text-white text-sm font-semibold outline-none focus:border-amber-500 transition-all mt-1" />
                  </div>
                  <div>
                    <label className="text-[9px] text-zinc-600 font-black uppercase tracking-widest ml-1">Date of Birth</label>
                    <input type="text" value={dob} onChange={e => setDob(e.target.value)} className="w-full py-3 px-4 bg-zinc-900 border border-zinc-800 rounded-xl text-white text-sm font-semibold outline-none focus:border-amber-500 transition-all mt-1" />
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 rounded-2xl p-5 border border-zinc-700/30 mb-6">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-zinc-500 text-[10px]">Primary Account</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest">Available Balance</p>
                    <h3 className="text-3xl font-black text-white tracking-tighter">${balance.toFixed(2)}</h3>
                  </div>
                </div>

                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-zinc-800/40 group/copy">
                    <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest">Wallet Address</span>
            ...
            {kyc ? (
              <>
                <div className="flex items-baseline gap-3 mb-1">
                  <h3 className="text-3xl font-black text-white tracking-tighter">${(totalSent + totalReceived).toFixed(2)}</h3>
                </div>
