...
const Request = () => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="h-screen w-full bg-black text-white flex items-center justify-center">
        <p className="text-gray-400 text-xl">No data found.</p>
      </div>
    );
  }

  return (
    <div className="h-screen w-full bg-black text-white p-5 border-t">
      {data.map((elem) => (
        <Reqpay key={elem._id} name={elem.name} sender={elem.sender} amount={elem.amount} />
      ))}
    </div>
  );
};
